...
`;
    ...
    for (const apiKey of shuffledApiKeys) {
      ...
      try {
        ...
        try {
          ...
          succeeded = true;
          console.log(
            `Successfully fetched news for ${region} with API key: ${apiKey.substring(
              0,
              10
            )}...`
          );
        ...
        console.error(
          `Error using API key ${apiKey.substring(0, 10)}... for ${region}:`,
          error
        );
        lastError = error;
    ...
    if (!succeeded) {
      console.error(
        `Failed to fetch news for ${region} with all API keys. Last error:`,
        lastError
      );
    }
  }
