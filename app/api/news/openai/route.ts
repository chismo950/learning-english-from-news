...
async function fetchNewsForRegions(
  ...
  console.log(
    `${
      skipCache ? "Skipping cache," : "Cache miss for " + redisKey + ","
    } fetching from AI...`
  );
...
Find the 5 latest news articles from ${
      region === "international" ? "international news" : region
    }.
...
${
  level === "intermediate"
    ? "- As this is for intermediate English learners, use vocabulary within the range of Wordly Wise 3000. Avoid complex or uncommon words outside this range."
    : ""
}
...
async function getCachedNews(
  ...
  const cachedFetch = unstable_cache(
    async () => {
      return await fetchNewsForRegions(
        language,
        regions,
        todayStr,
        level,
  ...
  // If result is empty array, fetch fresh data without cache
  if (result.length === 0) {
    console.log("Cached result is empty, fetching fresh data...");
    return await fetchNewsForRegions(language, regions, todayStr, level, true);
  }
  ...
  try {
    ...
    if (
      ...
      return Response.json(
        { error: "Language, regions, and valid level are required" },
        { status: 400 }
      );
    }
